...
{   // zeigt alle offenen winboxes
    ...
    console.log("Gefundene Fenster:", winboxElements.length);
    const parentIds = Array.from(winboxElements).map(el => {
        const parentWithId = el.closest("div[id]");
        return parentWithId ? parentWithId.id : "keine ID";
    });
    return parentIds;
}

function eL_show_hide_menue() 
{   // zentrale Menuesteuerung
    const menuToggle = document.querySelector('.menu-toggle');
    const mainNav = document.querySelector('.main-nav');
    const menuLinks = document.querySelectorAll('.main-nav a'); // Alle Links im Menü auswählen
    ...
    menuLinks.forEach(link => {
        link.addEventListener('click', function () {
            if (mainNav.classList.contains('show')) {
                mainNav.classList.remove('show'); // Menü verstecken, wenn es sichtbar ist
            }
        });
    });
}

function activate_menue() 
{   // Menue-Click-Steuerung
    const menuItems = document.querySelectorAll(".main-nav > li");
    menuItems.forEach(li => {
        const link = li.querySelector("a");
        const submenu = li.querySelector(".submenu");
        // Menü öffnen/schließen per Klick auf den Hauptpunkt
        if (submenu) {
            ...
            // Unterpunkt klick → Menü schließen
    ...
    // Optional: Klick außerhalb schließt alle Menüs
...
async function load_file_output(dateiname) {
...
function winbox_url(data) 
{   // Zentrale Winbox-Steuerung
    ...
    const data_a = JSON.parse(data);
    // console.log(data_a);
    const num = data_a.num ?? 59595959;
    if (parseInt(num) < 1000) {
        const exisiting_winbox = document.getElementById("winbox-" + num);
        if (exisiting_winbox) exisiting_winbox.winbox.close();
    }
    let url = data_a['url'];
    url = addQuery(url, "num", num);
    url = addQuery(url, "work_mode", data_a['work_mode']);
    url = addQuery(url, "fid_str", data_a['fid_str']);
    url = addQuery(url, "limit", data_a['limit']);

    var wb = new WinBox(data_a['title'], {
        id: "winbox-" + num,
        url: url,
        class: ["no-full", "md"],
        icon: window.top.miq_root_path + "img/ai.svg",
        top: window.top.men_height,
        overflow: true,
        onmove: function (x, y) { },
    ...
    // if (data_a['pos']) {
    //     wb.move(eval(data_a['pos'][0]), eval(data_a['pos'][1]));
    //     wb.resize(eval(data_a['pos'][2]), eval(data_a['pos'][3]));

    // }
    if (window.top.win_boxes[num]) {
        if (window.top.win_boxes?.[num]?.pos) {
            wb.move(window.top.win_boxes[num]['pos'][0], window.top.win_boxes[num]['pos'][1]);
        }
        if (window.top.win_boxes?.[num]?.size) {
            wb.resize(window.top.win_boxes[num]['size'][0], window.top.win_boxes[num]['size'][1]);
        }
    }
...
function load_fg_frame(fg, fcid, param_str, act_work_frame, name_file, helper = '') 
{   // Hilfsfunktion zum laden von frames 
    let helper_str = '';
    if (helper) helper_str = '&helper=' + helper;
    act_work_frame.src = name_file + '?fg=' + fg + '&fcid=' + fcid + helper_str + '&param_str=' + btoa(encodeURIComponent(param_str));
};

// Die beiden nachfolgenden Funktionen machen ziemlich das Gleiche sind historisch durch verschiedene
// Anwendungsfälle entstanden und sollten zusammegeführt werden. TODO: 3
function findClosestWinboxFromIframe(iframeEl) {
    // Aufruf: const winbox = window.top.findClosestWinboxFromIframe(window.frameElement);
    let current = iframeEl;
    while (current) {
        if (
            current.id &&
            current.id.startsWith("winbox-")
        ) {
            return current;
            console.log(current);
        }
...
function findParentWinboxDiv(this_window) {
    if (window.parent === this_window) {
        return null;
    }
    ...
    for (const iframe of iframes) {
        try {
            if (iframe.contentWindow === this_window) {
    ...
    if (containingIframe) {
        return containingIframe.closest('div[id^="winbox-"]');
    }
    return null;
}

function set_last_winbox_state(num, this_winbox) 
{   // Speichert die Postion der Winwboxes
    if (this_winbox){
        let cs = window.getComputedStyle(this_winbox);
        let topPos = parseInt(cs.top, 10);
        let leftPos = parseInt(cs.left, 10);
        let widthE = parseInt(cs.width, 10);
        let heightE = parseInt(cs.height, 10);
        set_win_box(num, 'pos', [leftPos, topPos]);
        set_win_box(num, 'size', [widthE, heightE]);
    }
    // console.log("Left:", leftPos, "Top:", topPos, "Width:", widthE, "Height:", heightE);
    // console.log(window.top.win_boxes);
}

function set_win_box(num, key, param) 
{   // Setzt die Eingenschaften der win_boxes-Info
    if (typeof window.top.win_boxes[num] !== 'object') {
        window.top.win_boxes[num] = {};   // falls undefined
    }
    window.top.win_boxes[num][key] = param;
}

function exit_button() 
{   // Separate Funktion für den Exit Button, da Menues sich nicht beim Überfahren öffenen sondern 
    // durch Klick, was Probleme bei mobilen Geräten impliziert, kann optimiert werden. TODO: 5
